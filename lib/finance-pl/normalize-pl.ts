import { readSheetRaw } from "@/lib/finance-pl/excel-utils";
import {
  GL_COL,
  GL_NUMBER_SEPARATOR,
  GL_SHEET_IDENTIFIER,
...
} from "@/lib/finance-pl/constants";
import type { NormalizePLResult, NormalizedRow, NormalizeWarning } from "@/lib/finance-pl/types";
...
export function extractLoanNumber(desc: string): string | null {
  // Prefer 12-digit standalone number, then 11, 10, 9 (all resolved later by completion)
  const m12 = desc.match(/(?<!\d)\d{12}(?!\d)/);
  if (m12) return m12[0];
  const m11 = desc.match(/(?<!\d)\d{11}(?!\d)/);
  if (m11) return m11[0];
