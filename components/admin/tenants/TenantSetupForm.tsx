...
import {
    ...
    Plus, Trash2, AlertCircle, CheckCircle2, Info, Globe, ChevronDown, XCircle,
} from "lucide-react";
...
import { addTenantAction, generateTCODEAction, getActiveHomesiEmployeesAction, EligibleEmployee } from "@/app/actions/tenant-actions";
...
export const TenantSetupForm: React.FC<TenantSetupFormProps> = ({ onClose, onSaved }) => {
    ...
    // All active employees — used for both POC picker and AM picker
    const [eligibleEmployees, setEligibleEmployees] = useState<EligibleEmployee[]>([]);
    // Track which POC slot has a linked employee (eid)
    const [pocLinkedEid, setPocLinkedEid] = useState<Record<number, string>>({});

    useEffect(() => {
        // Fetches ALL active employees across all tenants (admin has global visibility)
        getActiveHomesiEmployeesAction().then(setEligibleEmployees);
    }, []);
    ...
    /**
     * When an employee is selected from the dropdown, auto-populate POC fields
     * from the employee record and lock First Name / Last Name / Email.
     */
    const selectPocEmployee = (index: number, eid: string) => {
        const emp = eligibleEmployees.find((e) => e.eid === eid);
        if (!emp) return;
        setPocLinkedEid((prev) => ({ ...prev, [index]: eid }));
        setForm((f) => {
            const pocs = [...f.pocs];
            pocs[index] = {
                ...pocs[index],
                first_name: emp.primer_nombre,
                last_name: emp.primer_apellido,
                corporate_email: emp.email_corporativo ?? "",
            };
            return { ...f, pocs };
        });
    };

    /** Clears the linked employee from a POC slot and resets editable fields. */
    const clearPocEmployee = (index: number) => {
        setPocLinkedEid((prev) => {
            const next = { ...prev };
            delete next[index];
            return next;
        ...
        setForm((f) => {
            const pocs = [...f.pocs];
            pocs[index] = { ...pocs[index], first_name: "", last_name: "", corporate_email: "" };
            return { ...f, pocs };
        });
    };
    ...
    const handleSave = async () => {
        ...
        try {
            const tenant_id = await generateTCODEAction();
            const tenant: Tenant = {
                ...form,
                tenant_id,
            ...
            const result = await addTenantAction(tenant);

            if (!result.success) {
                setSaveError(result.error ?? "Failed to save tenant. Please try again.");
                return;
            }
            ...
            console.error("[TenantSetupForm] Save error:", err);
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden bg-white">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
                ...
                <section>
                    <div className="flex items-center justify-between mb-5">
                        <SectionHeader
                            ...
                            subtitle={`Client-side contacts (up to 10). ${form.pocs.length}/10 added.`}
                    ...
                    {eligibleEmployees.length === 0 && (
                        <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2">
                            ...
                            <p className="text-xs text-amber-700 font-medium">
                                No active employees found in HR Master HC (HOM-CO-01). Create employees via HR → Employee Intake first, then assign them as Points of Contact.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        {form.pocs.map((poc, i) => {
                            const linkedEid = pocLinkedEid[i];
                            const isLinked = !!linkedEid;
                                ...
                                <div key={poc.id} className="relative bg-slate-50 rounded-xl p-4 border border-slate-100">
                                    ...
                                    <div className="mb-3">
                                        <label className="block text-xs font-semibold text-slate-600 mb-1">
                                            Select Employee <span className="text-slate-400 font-normal">(from HR Master HC)</span>
                                        </label>
                                        {isLinked ? (
                                            <div className="flex items-center gap-2 px-3 py-2 bg-cobalt-blue/5 border border-cobalt-blue/20 rounded-lg">
                                                <div className="flex-1">
                                                    <p className="text-sm font-semibold text-navy-blue">
                                                        {poc.first_name} {poc.last_name}
                                                    </p>
                                                    <p className="text-xs text-slate-400 font-mono">{linkedEid}</p>
                                                </div>
                                                <button
                                                    type="button"
                                                    onClick={() => clearPocEmployee(i)}
                                                    className="flex items-center gap-1 text-xs text-slate-500 hover:text-action-red transition-colors"
                                                >
                                                    <XCircle className="w-3.5 h-3.5" /> Change
                                                </button>
                                            </div>
                                        ) : (
                                            <select
                                                value=""
                                                onChange={(e) => {
                                                    if (e.target.value) selectPocEmployee(i, e.target.value);
                                                }}
                                    ...
                                    {/* Auto-populated identity (read-only when linked) + editable contact fields */}
                                    <div className="grid grid-cols-3 gap-3">
                                        <Field
                                            label="First Name"
                                            required={!!(poc.last_name || poc.corporate_email)}
                                            error={errors[`poc_${i}_first_name`]}
                                        >
                                            {isLinked ? (
                                                <div className="px-3 py-2 text-sm bg-slate-100 border border-slate-200 rounded-lg text-slate-600 font-medium select-none">
                                                    {poc.first_name}
                                                </div>
                                            ) : (
                                                <input
                                                    ...
                                                    className={inputCls(errors[`poc_${i}_first_name`])}
                                                    placeholder="First"
                                                />
                                            )}
                                        </Field>
                                        <Field label="Last Name" error={errors[`poc_${i}_last_name`]}>
                                            {isLinked ? (
                                                ...
                                                <input
                                                    ...
                                                    className={inputCls(errors[`poc_${i}_last_name`])}
                                                    placeholder="Last"
                                                />
                                            )}
                                        </Field>
                                        ...
                                        <Field
                                            ...
                                            error={errors[`poc_${i}_corporate_email`]}
                                            className="col-span-2"
                                        >
                                            {isLinked ? (
                                                <div className="px-3 py-2 text-sm bg-slate-100 border border-slate-200 rounded-lg text-slate-500 font-mono select-none">
                                                    {poc.corporate_email || "—"}
                                                </div>
                                            ) : (
                                                <input
                                                    type="email"
                                                    value={poc.corporate_email}
                                                    onChange={(e) => setPOC(i, "corporate_email", e.target.value)}
                                                    className={inputCls(errors[`poc_${i}_corporate_email`])}
                ...
                <section>
                    ...
                    {eligibleEmployees.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            {eligibleEmployees.map((emp) => {
                                const fullName = [emp.primer_nombre, emp.primer_apellido].filter(Boolean).join(" ");
                ...
                {/* Save error */}
                {saveError && (
                    <div className="bg-red-50 border border-action-red/30 rounded-xl px-4 py-3 flex items-start gap-2">
                        <AlertCircle className="w-4 h-4 text-action-red mt-0.5 shrink-0" />
                        <div>
                            <p className="text-xs font-bold text-action-red">Failed to save tenant</p>
                            <p className="text-xs text-action-red/80 mt-0.5">{saveError}</p>
                        </div>
                    </div>
                )}
            </div>
