...
import {
    Plus, X, Save, Send, CheckCircle2, Clock, Power, PowerOff,
...
export const JobTitleApp: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        const res = await saveJobTitleAction({ ...form, id: editId ?? undefined, jdf_data: jdf });
        setSaving(false);
        if (res.success) {
    ...
    // Approver options: exclude self-referential selections
    ...
    return (
        <div className="flex flex-col h-full bg-white relative">
            ...
            <div className="px-6 py-5 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
                <div>
                    <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <h2 className="text-xl font-black text-navy-blue">Job Title Manager</h2>
                    </div>
                    <p className="text-sm text-slate-500">Create and manage your organization's official Job Titles and Job Description Forms.</p>
                </div>
            ...
            <div className="px-6 py-3 border-b border-slate-100 bg-slate-50 flex items-center gap-4 shrink-0">
                ...
                <div className="flex items-center gap-4 ml-auto text-xs font-bold text-slate-500">
                    <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" /> {jobTitles.filter(j => j.status === "Active").length} Active</span>
                    <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> {jobTitles.filter(j => j.status === "Draft").length} Draft</span>
                    <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-slate-300 inline-block" /> {jobTitles.filter(j => j.status === "Inactive").length} Inactive</span>
                </div>
            </div>
                                ...
                                <tr key={jt.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-5 py-3.5 text-right">
                                        <div className="flex items-center gap-2 justify-end">
                                            <button onClick={() => openEdit(jt)} className="text-xs text-cobalt-blue hover:underline font-bold">Edit JDF</button>
                                            <button
                ...
                <div className="fixed inset-0 z-50 flex">
                    ...
                    <div className="w-full max-w-2xl bg-white h-full flex flex-col shadow-2xl overflow-hidden">
                        ...
                        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
                            <div>
                                <h3 className="font-black text-navy-blue text-lg">{editId ? "Edit Job Description Form" : "New Job Title"}</h3>
