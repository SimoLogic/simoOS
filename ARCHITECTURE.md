...
- **NUNCA** mostrar UI de borrado para tareas protegidas
- **NUNCA** ejecutar DELETE en API para tareas protegidas
- **DB Trigger Guard** bloquea igualmente a nivel base de datos
- Registrar `SecurityEvent` en CADA intento de borrado bloqueado
- Vectores cubiertos: HTTP request, Prisma direct call, raw SQL
...
| Backend | Node.js 20 + Express 5 + TS · Prisma ORM + Middleware Protect · Zod · Bull Queue · Socket.io · JWT |
| DB | PostgreSQL 16 (source of truth + RLS al final) · Redis 7 (Speed Layer) · ElasticSearch 8 (full-text) · MinIO/S3 (archivos) |
...
- Métodos: `addWorkdays(start, n, holidays[], timezone)`, `isWorkday(date)`, `nextWorkday(date)`
- Tests unitarios obligatorios antes de avanzar (Sprint 2)

### 🗝️ Llave #3 — DB Trigger Guard
- **Capa 1:** Middleware Prisma intercepta `delete/deleteMany` en Task con `sourcePlaybookId !== null`
- **Capa 2:** Trigger PostgreSQL `BEFORE DELETE ON tasks` rechaza si `source_playbook_id IS NOT NULL`
- **Capa 3:** API retorna 403 + `TASK_PLAYBOOK_PROTECTED` + registra `SecurityEvent`
- Protege contra: HTTP, Prisma directo, psql/scripts SQL, curl malicioso

### 🗝️ Llave #4 — Mirror Sync Protocol
- `POST /api/integrations/simo/playbook-task-update`
- Solo actualiza: `title`, `description`, `dueDate`, `priority` (campos padre)
- NUNCA toca: `subtasks[]`, `comments[]`, `attachments[]`, `customFieldValues[]`
- Conflicto de `status` → Modal de resolución para el empleado
- Log en tabla `sync_events`
